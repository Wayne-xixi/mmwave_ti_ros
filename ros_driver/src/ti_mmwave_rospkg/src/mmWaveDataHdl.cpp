...
void mmWaveDataHdl::onInit()
{
    ...
    if (!(private_nh.getParam("radar_bin", myRadarBin))) {
        myRadarBin = "generic";  // Use generic if none specified
    }
    ...
    DataUARTHandler DataHandler(&private_nh, (char*) myRadarBin.c_str());
